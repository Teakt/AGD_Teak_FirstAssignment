...
public class KinematicC : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (System.Math.Round(transform.rotation.y, 2) != System.Math.Round(Quaternion.LookRotation(transform.position - target.transform.position).y, 2))
            {
                ...
                //Debug.Log("Rota y " + System.Math.Round(transform.rotation.y, 2) + " and " + System.Math.Round(Quaternion.LookRotation( transform.position - target.transform.position).y, 2));
            }
            else
            {
                Debug.Log("Outside Near Radius " + distanceFromTarget + " Velocity " + _velocity);
                transform.position += transform.forward   * Time.deltaTime * lowVelocity;
            }
